...
| `CollabChatPanel` | ~280 | Collab — extracted reusable chat surface (scroll + input bar + `KeyboardPersistentTextView`). `showAvatarHeader` opt-in renders a peer-only avatar at top with "stepped away" dim + moon badge. Drag-down + scroll-down both dismiss keyboard. Custom iMessage-Tapback-style reaction picker (long-press a peer bubble → six emoji + X dismiss in a horizontal capsule, sibling of the bubble in a per-row VStack). Used by `PlanSuggestionView` AND `WorkoutChatToolbar`. |
...
- `Forge/Views/Collab/`:
  ...
  - `CollabChatPanel.swift` — extracted reusable chat surface (scroll + bubble + input bar) + `KeyboardPersistentTextView` (UIViewRepresentable wrapping `WrappingUITextView`, a UITextView subclass returning `noIntrinsicMetric` for width so long lines wrap instead of expanding the bar). `showAvatarHeader: Bool` opt-in renders peer-only avatar at the top with the away dim treatment. `simultaneousGesture(DragGesture)` + `.scrollDismissesKeyboard(.immediately)` both dismiss keyboard on downward drag. **Custom reaction picker:** long-press a peer bubble (0.3s) fires a medium haptic + spring-animates a horizontal emoji capsule (six iMessage Tapbacks + X dismiss) into view as a SIBLING of the bubble in a per-row VStack — sits in normal layout flow below the bubble so the next message is naturally pushed down. Tap an emoji to apply (or toggle off if same as current); tap X to dismiss. Reaction badge is a `Text(emoji).background(Circle())` with `.offset(x: ±12, y: 14)` over the bubble corner; LazyVStack spacing 16 clears the protrusion. Picker emoji buttons are plain `Text` + `.onTapGesture` (NOT `Button` — see gotcha). Used by both `PlanSuggestionView` and `WorkoutChatToolbar`.
...
- **Don't use `.contextMenu` for the chat reaction picker.** We tried; the native lift/dismiss pipeline produced a steady stream of platform-specific glitches that we couldn't reach from SwiftUI: badge clipping during the lift snapshot (the snapshot clips at the receiver's layout bounds, so any `.offset` on the badge gets cut off), iOS 18 bubble wobble during the dismiss transaction (the dismiss animation runs at the UIKit/system layer outside SwiftUI's transaction propagation, so `.animation(nil, value:)` and `.transaction { animation = nil }` can't reach it — only fix is to keep the receiver's frame constant via always-on padding), behind-the-bubble flash on stamp (the badge sits in the regular layer below the elevated platter, so it renders behind the bubble during the dismiss until the platter unwinds), and rounded-corner flicker as the platter completes. Each fix exposed another. The custom long-press picker (`onLongPressGesture` + sibling capsule view in a per-row VStack — see `CollabChatPanel`) sidesteps all of it because there's no platter, no system snapshot, no separate transaction layer. **Don't "fix" this back to native.**
- **Reaction picker emoji buttons MUST be `Text` + `.onTapGesture`, not `Button`.** Even with `.buttonStyle(.plain)`, iOS's button foreground tint absorbs colored emoji glyphs and renders them invisible — only the X button (an SF Symbol with explicit `.foregroundColor(.white)`) shows up. Plain `Text` + tap gesture has no inherited tint, so emoji glyphs render in their natural colors.
- **Chat reactions wire protocol shares a `messageId` between both peers.** `ChatEntry.id` is the shared id — the sender mints it once at `sendChat`, the server forwards it inside `peerChat`, and both peers key the same chat row. This lets `setReaction` / `peerReactionChanged` address a specific message across the wire. Old wire shape (no `messageId`) is incompatible — server redeploy + app rebuild in lockstep.
...
- **`UIScreen.main.bounds`** is still used in `WorkoutInProgressView` (`screenHeight * 0.8` break-timer panel expansion + `isSmallScreen` check + chat-panel sizing), `BreakTimerView` (ring diameter), `ConfettiView` (particle x-distribution), `WorkoutWithFriendView` (≥414pt width gate for the secondary-instruction divider), `ExerciseEditorView` (title-bar 0.2/0.6/0.2 split). Deprecated in iOS 16+ but doesn't warn at our deployment target. Migrating to `GeometryReader` is the B5 task in the multi-iPhone-size UI refactor — see "Future roadmap".

- **Small-screen UX swap pattern (`isSmallScreen`).** `WorkoutInProgressView` defines `isSmallScreen: Bool { screenHeight < 700 }` (true on SE-class). On these phones the break-timer panel can't fit the in-ring X cancel button beneath the ring + Up Next + safe-area padding, so the disabled back chevron at top-left swaps to an `xmark` that calls `dismissBreakTimerView()` while the timer is active, and the in-ring X hides. Reverts to chevron-left when the timer dismisses/expires. `isSmallScreen` is passed down to `BreakTimerView` as a parameter (not recomputed there) so the parent's screen-size determination flows through consistently.

- **Content-aware sheet detent in `ExerciseEditorView`.** Sheet uses `.presentationDetents([homoModeDetent, .large])` where `homoModeDetent` is `.height(360 + buffer)`. `buffer` is the bottom safe-area inset (read once from `UIApplication.shared.connectedScenes`'s key window) on home-indicator phones, or a fixed 40pt on SE — because SE has no home-indicator visual area for content to bleed into, so any vertical overflow hard-clips at the sheet edge. Detent ownership lives entirely inside `ExerciseEditorView` (no `selectedDetent` binding from parents) so the height calc lives next to the content it sizes; the toggle's `onChange` and `onAppear` set `selectedDetent = newValue ? .large : homoModeDetent` directly.
...
- [ ] **Mid-workout chat enhancements** — the chat panel + bubble reactions are shipped; remaining chat features:
...
- [~] **UI refactor: multi-iPhone-size support** — partial. Target range: iPhone SE 3rd gen (4.7" / 375pt) through 17 Pro Max (6.9" / 430pt), portrait-only.
  - **Shipped:** Phase A constant annotations (load-bearing animation endpoints documented inline). B1 — `WorkoutBottomToolbarView` rebuilt with `maxWidth: .infinity` columns (no `UIScreen.main`). B2 — `minimumScaleFactor` + `lineLimit(1)` on ≥30pt fonts in `BreakTimerView` / `StartingCountdownView` / `PlanEditorView` / `HistoryView`. B3 — `ExerciseEditorView` content-aware `.height(N)` sheet detent + relative TextField. Plus ad-hoc: `BreakTimerView` ring sizing (200pt SE / scaled to 260pt cap on others) + small-screen back-button-X swap, `WorkoutWithFriendView` divider/secondary-text gated to ≥414pt, `ExerciseEditorView` "Change Specific Sets" toggle width (235→280), bottom-toolbar centering. Reduced outer horizontal padding 40→24 on `StartingCountdownView` + `BreakTimerView`.
  - **Outstanding:** B4 (`WorkoutWithFriendView` Pro Max vertical-spacing balance — paused, will be done manually). B5 (replace remaining `UIScreen.main` usages — see the gotcha line above for the 5 sites). B6 (visually verify `SetView` strikethrough alignment on SE / Pro Max).
  - **Known issue:** break-timer panel still uses `topToolBarHeight = screenHeight * 0.8`, leaving excess empty space below the cancel button on Air / Pro Max. Content-aware panel-height was attempted but broke the expansion animation (the 75 + 163pt plan-info section above `BreakTimerView` competes for the same panel height; sizing for `BreakTimerView` content alone caused mid-frame center-clipping). Needs a different approach — e.g. layoutPriority + capping `BreakTimerView`'s internal greedy `Spacer().frame(maxHeight:)`.
...
The codebase has gone through ~18 stages of restructuring: string→enum modes, `WorkoutInProgressView` decomposition, `ExerciseEditorView` decomposition + Int picker state, deprecated-API replacement, dark-mode enforcement, Live Activity, HealthKit + calorie capture, Apple Watch companion, Settings + theming refactor, plan sharing, the full collab feature (sessions, profiles, plan suggestion, chat, ready/start, set sync, avatar gutter, break-timer sync, heartbeat + auto-reconnect, peer-disconnect banner, mid-workout rejoin), post-collab hardening (bug roll-up, `SessionClientTests`, `Log.debug`, `GlobalSettings` constants, `.connected → .paired(peerIds:)` invariant, conservative `WorkoutInProgressView` decomposition), the **mid-workout chat panel** (extracted `CollabChatPanel` + new `WorkoutChatToolbar` + multi-line `KeyboardPersistentTextView` + manual keyboard avoidance + animated rounded-top expansion), **background-aware peer presence** (stable per-device participant ID + server-side `.away` slot lifecycle with 5-min timeout + `peerAway`/`peerReturned` protocol + subtle dimmed-avatar UI), **multi-iPhone-size UI refactor (partial)** — Phase A constant annotations + B1-B3 surgical fixes (relative widths in `WorkoutBottomToolbarView`, `minimumScaleFactor` on large fonts, content-aware sheet detents in `ExerciseEditorView`, small-screen back-button-X swap during break timer), and **chat bubble reactions** (iMessage Tapback-style: `sendChat`/`peerChat` extended with shared `messageId`, new `setReaction` / `peerReactionChanged` wire cases — pure relay, no server state — `ChatEntry.myReaction`/`peerReaction` fields, custom long-press picker as a sibling of the bubble in a per-row VStack after a long native-`.contextMenu` detour proved untenable across iOS 18/26). See git log for `Stage N` commits and dated commits for detail.
