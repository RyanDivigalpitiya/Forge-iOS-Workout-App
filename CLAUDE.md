...
- `WorkoutPlan` → `[Exercise]` → `[Set]` (weight, reps, tillFailure, completed). All `Identifiable + Codable` with UUIDs. `WorkoutPlan` also carries `lineageId: UUID?` (stable identity preserved through `importPlan` and collab auto-import) and `fingerprint: String?` (SHA-256 over ordered `(lowercased+trimmed name, setCount)` pairs via `WorkoutPlan.computeFingerprint(for:)`; auto-refreshed via `exercises.didSet`; defines structural equality). Both optional for backward-compat decode; `migrateLineageAndFingerprintIfNeeded()` in `PlanViewModel.init()` backfills live records. `PlanSnapshot` (wire) carries both fields so receiver can run `findMatch`.
...
| `HistoryView` | Read-only past workout — segmented tabs: Most Recent (calories/completion/duration + per-exercise set-completion render) and Full History (per-exercise progress cards with Weight PR / Reps PR + Weight↔Reps line chart via SwiftUI Charts) |
...
- `Forge/View Model/SessionClient.swift` — `@MainActor` ObservableObject. State: `.idle`/`.connecting`/`.waitingForPeer`/`.paired(peerIds:)`/`.disconnected(reason:)`/`.error(_)`. Wire types: `Profile`, `PeerInfo`, `PlanSnapshot`, `ChatEntry`, `PeerSetKey`, `UserPosition`, `PeerBreakTimer`, `PeerExitInfo`. Owned by `CompletedWorkoutsView` as `@StateObject`, injected at NavigationStack root. See gotchas for stable participant ID, heartbeat, auto-reconnect, await-flush.
- `Forge/Views/Collab/`:
  - `WorkoutWithFriendView.swift` — `activeWorkoutPlan: WorkoutPlan?` switches: nil (History) → `createSession()` + push `ConnectingView`; non-nil (mid-workout) → `startSharedSessionForActiveWorkout(plan:)` + dismiss. Single `.sheet(item: $activeSheet)` covers share sheet AND workout-mode profile prompt. When host has no profile, presents `JoinSessionView(onSoloProfileSaved:)`; closure dismisses then re-fires action one runloop tick later (UIKit hop).
  - `JoinSessionView.swift` — name + `PhotosPicker`. `autoSubmitIfCachedProfile()` skips form for returning users. `onSoloProfileSaved` activates solo-profile-entry mode (hides peer half, button "Save", title "Your Profile", skips auto-submit). `@FocusState` auto-focuses empty name field 0.4s after appear.
  - `PlanSuggestionView.swift` — carousel + `CollabChatPanel()` + Ready + single enum-driven `.fullScreenCover(item:)` for `PlanEditorView` (preview) and `WorkoutInProgressView` (post-ready). `resolveCoverState()` runs match-and-route on appear + `workoutInProgress.id` change so re-joiners auto-route into active workout.
  - `CollabChatPanel.swift` — chat + `KeyboardPersistentTextView` (UIViewRepresentable around `WrappingUITextView`, a UITextView returning `noIntrinsicMetric` for width). `showAvatarHeader` opt-in. Custom reaction picker: long-press (0.3s) → spring-animates horizontal emoji capsule (six Tapbacks + X) as sibling of bubble in per-row VStack. Badge `Text(emoji).background(Circle()).offset(x:±12,y:14)`; LazyVStack spacing 16 clears protrusion.
  - `CollabStatusBanner.swift` — capsule overlay. Hidden in `.paired`/`.idle`. `.waitingForPeer` shows "Re-invite" that share-sheets the EXISTING `sessionId` URL (no `createSession()`). Clean-exit override: `peerExitInfo` → "Friend finished/left workout" for ~5s then auto-dismisses; presence suppresses disconnect chrome for rest of session. Doesn't surface peer backgrounding (avatar-level only).
  - `PlanCarouselCard.swift`, `PlanInfoBlock.swift` — shared plan-display pieces.
  - `CollabHelpers.swift` — `avatar`, `initial`, `resizeImage`, `ShareSheet`, `ShareableURL`, `awayDimmedAvatar` (opacity 0.45 + moon badge for chat header; workout gutter uses opacity-only).
- `Forge/Views/History/CompletedWorkoutsView.swift` — Add Friend toolbar, `forge://session/<id>` in `.onOpenURL`. `scenePhase`: `.active` → `reconnect()` + `sendReturningToForeground()`; `.background` → awaited `sendGoingBackgroundAndAwait()`. `.onChange(of: state)` resets nav flags on `.idle`. Closes NavigationStack with `.environmentObject(sessionClient)`.
- `Forge/Views/Workout/WorkoutInProgressView.swift` — `@EnvironmentObject var sessionClient`. Joint mode: per-exercise cards reserve a leading gutter; `.overlayPreferenceValue(RowAnchorKey.self)` absolutely positions an `avatarColumn` per row at its measured `midY` (wrap-resilient). Peer avatars dim to 0.45 when `peerAway[peerId] != nil`. `myPosition` is `@State` written via `recomputeMyPosition()`. Both `.onChange(of: state) { case .paired }` AND `.onChange(of: peerAway)` non-empty→empty call `rebroadcastJointStateForPeer()`. `finishWorkout`/`cancelWorkout` use async `sendAndAwait`; disconnect deferred inside 2s confetti. Mid-workout invite button visible when `sessionId == nil` OR `peerExitInfo != nil`. Mid-workout chat via `WorkoutChatToolbar` ZStack-sibling of `WorkoutBottomToolbarView`. Anchor-preference types in `WorkoutAvatarGutter.swift`.
...
- **Joint-mode set sync is positional: `(exerciseIndex: Int, setIndex: Int)`.** Phase C: each peer may use their own local plan with their own UUIDs, so positional is the only encoding that resolves to "the same exercise" on both phones. `peerSetCompletion` wire and `PeerSetKey` storage carry no UUIDs. `PlanSnapshot.toWorkoutPlan()` still copies UUIDs (cosmetic, for read-only preview). `SetSnapshot` has no id either.
...
- **`@EnvironmentObject` does NOT auto-propagate from `@StateObject` parent** — must explicitly `.environmentObject(...)`. SessionClient lives as `@StateObject` on `CompletedWorkoutsView`; inject ONCE on NavigationStack so all destinations + modals inherit.
...
- **After every `.paired` transition, re-broadcast joint state.** Server only persists `suggestedPlan` + `workoutInProgress`, not position/sets/break-timer. `WorkoutInProgressView`'s `.onChange(of: state) { case .paired }` calls `rebroadcastJointStateForPeer()`, which iterates `activePlan.exercises.enumerated()` and emits each completed set as positional `setCompletion(exerciseIndex:, setIndex:, completed:true)`. Symmetric, idempotent. Away-return needs the SECOND `peerAway`-emptied observer (separate gotcha) since still-paired peer's `state` doesn't transition.
...
- **Mid-workout rejoin needs server-side `workoutInProgress` tracking.** Without it, a rejoiner can broadcast a fresh `startWorkout` and stomp partner's active workout. Server records on all-ready transition, persists for session lifetime, sends in `welcome`. iOS routes rejoiner straight into `WorkoutInProgressView`, skipping suggest/Ready.
...
- **Reaction picker emoji must be `Text` + `.onTapGesture`, not `Button`.** Even with `.buttonStyle(.plain)`, iOS's button tint absorbs colored emoji glyphs and renders them invisible. `Text` + tap has no inherited tint.
...
- **Phase C same-plan-detection routing.** `PlanViewModel.findMatch(forFingerprint:lineageId:)` returns `.fingerprintMatch` (silent reuse of B's local plan, preserves B's weights/reps), `.lineageMatch` (divergence sheet — `DivergenceSheet.swift`), or `.none` (auto-import via `importPlan`). `PlanSuggestionView.routeIntoActiveWorkout(snapshot:)` switches on it. Auto-import branches set `lastAutoImportedPlanName` → `CompletedWorkoutsView` shows a 4s capsule toast. **Divergence sheet has only "use friend's version" options — no "use my version".** Positional sync requires shared structure; offering "use mine" would silently disable peer set sync, which is worse UX than the constraint. Options: ephemeral (read-only snapshot, `activePlanIndex = -1`) or persistent (import as new plan with name suffix). **Phase C subsumed the Option-1 sentinel fix:** routing always sets a valid `activePlanIndex`, so the prior stale-index destructive overwrite at `WorkoutInProgressView.finishWorkout`/`cancelWorkout` is unreachable. Don't reintroduce a write site that bypasses this routing.
...
- [x] **Plan identity layers + same-plan collab + progress history** — `lineageId`/`fingerprint` on `WorkoutPlan` (Phase A); collab same-plan detection + positional set sync + divergence sheet (Phase C); per-exercise progress visualization in `HistoryView` Full History tab (Phase B).
- [ ] **Canonical exercise registry** — speculative; revisit if users complain that the same exercise across independently-created plans fragments progress.
