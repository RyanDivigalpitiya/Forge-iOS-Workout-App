...
| 6c | Break-timer sync + animations + visual polish | **current focus** |
...
### Current focus — Stage 6c

Break-timer sync + joint-workout polish. Stages 6a (dual checkboxes) and 6b (avatar column) landed; what's left is making the break timer itself aware of the peer — right now each phone's `BreakTimerView` runs independently. Also: animations between avatar-row positions so the avatar slides down to the rest-break row rather than disappearing-and-reappearing; between-exercise rest UI (currently no explicit row exists, so during a cross-exercise rest the avatar jumps forward to the next exercise's first set).

Implementation sketch:
- Extend protocol with a `ClientMessage.breakTimerUpdate(endDate:exerciseIndex:setIndex:)` and `ServerMessage.peerBreakTimerChanged(peerId:endDate:exerciseIndex:setIndex:)`. `endDate: Date?` — nil means "break ended / cancelled".
- Client sends on: timer-start (when `breakTimerEndDate` becomes non-nil) and timer-dismiss / expiry (when it becomes nil).
- `SessionClient` gains `@Published var peerBreakTimer: [UUID: PeerBreakTimer]` (or similar).
- `BreakTimerView` gets a small peer-timer indicator (maybe a muted parallel countdown + avatar) when the peer is also resting.
- Optional: matchedGeometryEffect or explicit `.transition(.slide)` on the avatar column so position changes animate smoothly.
- Optional: add an explicit between-exercise rest row to the layout so mid-exercise-transition rest has a place for the avatar.
...
- `Sources/ForgeServer/Protocol.swift` — `ServerMessage` / `ClientMessage` enums + `Profile` / `PeerInfo` / `PlanSnapshot` / `ExerciseSnapshot` / `SetSnapshot` value types. All Codable + Sendable. Incompatible-upgrade territory: every protocol change requires both server redeploy and app rebuild in lockstep.
- `Sources/ForgeServer/SessionManager.swift` — actor holding `[UUID: Session]`; lazy-creates sessions on first `addParticipant`; deletes when empty. Capacity = 2. Per-session state now includes: `suggestedPlan: PlanSnapshot?` + per-participant `isReady: Bool`. Server returns `true` from `setReady` when both phones just hit ready so the caller can broadcast `startWorkout` immediately (no authoritative countdown — `WorkoutInProgressView` handles that locally with `StartingCountdownView`).

**iOS (Forge target):**
- `Forge/View Model/SessionClient.swift` — `@MainActor` ObservableObject. `createSession()` / `joinSession(id:)` / `reconnect()` / `disconnect()` / `submitProfile(_:)` / `suggestPlan(from: WorkoutPlan)` / `sendChat(text:)` / `toggleReady()` / `sendSetCompletion(...)` / `sendPositionUpdate(_:)`. Mirrors all the wire types (`Profile`, `PeerInfo`, `PlanSnapshot`, `ChatEntry`, `PeerSetKey`, `UserPosition`) and `@Published`s the session-scoped state (`peerProfiles`, `suggestedPlan`, `chatEntries`, `peerReady`, `startWorkoutSignal`, `peerCompletedSets`, `peerPositions`). Owned by `CompletedWorkoutsView` as `@StateObject` — deliberately NOT on `ForgeApp`. On `handle(.welcome)` auto-resends the current profile if `hasSubmittedProfile` is true. `PlanSnapshot.toWorkoutPlan()` preserves UUIDs on the plan and each exercise (see gotcha below — critical for cross-phone coordination in joint mode).
- `Forge/Views/Collab/` (synchronized group) — one file per view now:
  ...
  - `JoinSessionView.swift` — name + `PhotosPicker` + peer banner
  - `PlanSuggestionView.swift` — the big one: carousel + chat + Ready buttons + `KeyboardPersistentTextField` (UIViewRepresentable wrapping UITextField) + single `.fullScreenCover(item:)` driven by an enum that covers both `PlanEditorView` (preview) and `WorkoutInProgressView` (post-ready hand-off)
...
- `Forge/Views/Workout/WorkoutInProgressView.swift` — gains `@EnvironmentObject var sessionClient: SessionClient`. In joint mode (`sessionClient.state == .connected`) the set-row HStack prepends an avatar column (who's at this set) + a grey peer-completion circle; the rest-row HStack prepends an avatar column (who's resting here) + a mirrored rest-break connector under the peer column. Set toggle broadcasts `sendSetCompletion`. Computed `myPosition` derives from plan completion state + break-timer flags; `.onChange(of: myPosition)` broadcasts `sendPositionUpdate`.
