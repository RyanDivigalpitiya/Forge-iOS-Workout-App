...
| `PlanSuggestionView` | ~530 | Collab — plan carousel + Suggest/Preview + chat (UIKit-backed `KeyboardPersistentTextField`) + indicator row (END SESSION / dot indicators / DISMISS) + auto-route into the active workout when `workoutInProgress` is set (Stage 7b'). Suggested-plan header renders as a card-style backdrop with a `TimelineView`-driven shimmer sweep on the plan name. Carousel has a trailing 75pt fade-to-black overlay that hides when the last card is current. |
| `CollabStatusBanner` | ~110 | Collab — capsule overlay shown on PlanSuggestionView + WorkoutInProgressView when state is non-healthy. In `.waitingForPeer` includes a "Re-invite" button that share-sheets the current sessionId URL (Stage 7b). Swipe-up drag gesture dismisses the banner for the current state; reappears when state changes. |
...
- `Forge/Views/Collab/` (synchronized group) — one file per view now:
  ...
  - `CollabStatusBanner.swift` — capsule overlay reading `sessionClient.state`. Hidden when `.paired`/`.idle`; shows context-appropriate text (+ spinner) for `.connecting`/`.disconnected`/`.error`/`.waitingForPeer`. In `.waitingForPeer`, includes a "Re-invite" button that share-sheets `sessionClient.shareLinkURL()` (the EXISTING sessionId — does NOT call `createSession()`). Embedded as `.overlay(alignment: .top)` on PlanSuggestionView and WorkoutInProgressView.
...
- `Forge/Views/Workout/WorkoutInProgressView.swift` — gains `@EnvironmentObject var sessionClient: SessionClient`. In joint mode (`sessionClient.isPaired`) each per-exercise card is wrapped in an HStack with an empty `Color.clear.frame(width: gutterWidth)` placeholder on the leading edge; an `.overlayPreferenceValue(RowAnchorKey.self)` reads each set/rest row's bounds anchor (published via `.anchorPreference`) and absolutely positions one `avatarColumn` per row at its measured midY. **No hardcoded row offsets** — wrap-resilient. `myPosition` is `@State` (not derived); `recomputeMyPosition()` applies a single rule: position = first-incomplete-set, with a rest-row adjustment when an active break timer's source set immediately precedes it. Called on every set tap, every break-timer dismiss, and workout start. `restingForSet: RestingSet?` tracks which set's break is running. The set-tap flow lives in `handleSetTap(exerciseIndex:setIndex:)` + `scheduleBreakTimerStart(exerciseIndex:setIndex:)` helpers; the Button action in the view body is a one-liner calling into them. Set toggle broadcasts `sendSetCompletion`; `.onChange(of: myPosition)` broadcasts `sendPositionUpdate`; `.onChange(of: sessionClient.state)` matching `case .paired` calls `rebroadcastJointStateForPeer()` which re-pushes my position + every completed set + active break-timer state — so a freshly-(re)paired peer sees my full state without waiting for my next action. `restBreakRow()` is wrapped in a `.plain` Button so any rest row (within- or between-set) is the tap target for `BreakDurationPickerView`. Anchor-preference types (`RestingSet`, `RowID`, `RowAnchorKey`) live in the sibling file `WorkoutAvatarGutter.swift`.
- `Forge/Views/Shared/SetView.swift` — `Appearance` enum has a `.workoutActiveCollab(isCompleted:)` case for joint-mode rows, used when `sessionClient.isPaired`. Renders weight/reps as a single grey chip (e.g. `100 lb x 12 reps`) with `.lineLimit(1)` so the compressed horizontal budget on small phones still fits. Set # label drops to 13pt + a narrower 46/54pt frame to match.
...
- **Stale per-peer state must be cleared on BOTH `reconnect()` AND `peerLeft`.** Server assigns a fresh `myId` on every WebSocket connection, so post-reconnect the peer has a new UUID. `peerProfiles` is cleared explicitly on both paths; peerIds clears automatically because it's now derived from `State.paired(peerIds:)` and the state transitions to `.disconnected` / `.waitingForPeer`. `peerPositions` / `peerBreakTimer` / `peerReady` (all keyed by peer UUID) were originally NOT cleared — old-UUID entries lingered and rendered as ghost "?" avatars next to the new peer. Fixed by mirroring the cleanup in both code paths. `peerCompletedSets` is keyed by `(exerciseId, setIndex)` (not peer UUID), so doesn't need this treatment.
- **After every transition into `.paired`, re-broadcast joint state.** Server doesn't persist position / completed sets / break-timer state — only the suggested plan and the `workoutInProgress` snapshot. So when a peer disconnects and reconnects (or a fresh peer joins), they have NO knowledge of my workout progress until I happen to do something. `WorkoutInProgressView`'s `.onChange(of: sessionClient.state)` calls `rebroadcastJointStateForPeer()` on every transition whose new value matches `case .paired`: pushes current position + every completed set + active break-timer state. Symmetric on both phones, idempotent.
...
**Mock data:** `Forge/View Model/MockData.swift` exposes `mockWorkoutPlans` and `mockCompletedWorkouts` as module-internal globals. Tests access them via `@testable import Forge`. The Preview-only mock init on `CompletedWorkoutsViewModel` used to reach into `completedWorkout2` directly, which made production depend on mock data; that's been removed — the mock init default-constructs `CompletedWorkout()` and the HistoryView preview sets `activePlan` explicitly if it needs richer sample data.
...
- **`SessionClient.State` carries peerIds in the `.paired` case.** There is no independent `@Published peerIds` property — the "connected without peers" illegal state is unrepresentable. Call sites read `sessionClient.isPaired` (bool convenience), pattern-match `case .paired(let ids) = state`, or use the `sessionClient.peerIds` computed accessor (returns `[]` in any non-paired state). Don't reintroduce a separate `peerIds` storage; it was removed in the Stage 5 invariant refactor to kill whole classes of view-state-vs-state-flag divergence bugs.
- **`Log.debug(_:)` is the project's logging channel.** New diagnostic traces in the Forge app target should call `Log.debug(...)` (defined in `Forge/View Model/Log.swift`), not `print(...)`. It takes `@autoclosure () -> String` and `#if DEBUG`-guards the print, so release builds don't pay for log-string interpolation. `ForgeWatch` still uses bare `print` — if that target ever gets its own `Log.debug`, mirror the same shape. For user-visible error surfaces, still use a banner / alert per the "Fail Loud, Never Fake" section below.
- **Prefer `GlobalSettings` constants over magic numbers for cornerRadius + animation durations.** `cornerRadiusSmall/Medium/Large` (5/8/16) cover chips, cards, and full panels respectively; `animationQuick/Standard/Slow` (0.2/0.5/1.0) cover quick UI feedback, view transitions, and opacity fades. Only use them when the semantic bucket matches — don't force idiosyncratic one-offs (e.g. 2s finish-workout fade, 12pt collab sub-panel border, 0.25s peerPosition anchor tween) into the shared scale just to avoid a literal.
- **`LinearGradient`'s `startPoint` / `endPoint` are NOT animatable via `withAnimation`.** Wrapping a gradient-endpoint change in `withAnimation(.linear...)` just snaps the gradient to its final state — no interpolation. For any continuously-animating gradient (shimmer, sweep, breathing glow), drive the phase from a `TimelineView(.animation) { context in ... }` that re-renders at the display refresh rate and recomputes endpoints from wall-clock time, e.g. `context.date.timeIntervalSinceReferenceDate.truncatingRemainder(dividingBy: cycleSeconds) / cycleSeconds`. The shimmer on the suggested plan name in `PlanSuggestionView` uses this pattern: a 2.5s cycle with UnitPoint phase mapped to `[-1.5, 1.5]` so the bright gradient peak sweeps from off-left across the text and exits off-right.

## Refactor history

The codebase has accumulated several rounds of refactoring (see git log for `Stage N` commits):
...
17. Post-collab hardening sprint (six-commit series on the `refactor` branch, see commits `81e3423`..`3c0a081`):
    - Bug roll-up: removed `breakTimerEndDate!` force-unwrap; `SessionClient.sendClientMessage` now logs encode failures and cancels the task with `.abnormalClosure` on send error so `readLoop` trips → auto-reconnect fires; production no longer reads the `completedWorkout2` mock global; `PlanViewModel.savePlans()` rejects plans with zero-set exercises; consolidated two racing `.onChange` handlers in `PlanSuggestionView` into a single `resolveCoverState()` with `workoutInProgress` precedence. Collab "Cancel Workout" in joint mode now calls `sessionClient.disconnect()` so the NavigationStack observer unwinds back to History (alert title reads "Leave Session?" in joint mode, "Cancel Workout?" in solo).
    - `ForgeTests/SessionClientTests.swift` added — 16 `@MainActor` Swift Testing cases covering every `ServerMessage` handler case. `SessionClient.handle(incoming:)` dropped `private` so tests feed wire JSON directly.
    - Hygiene: `Forge/View Model/Log.swift` (`Log.debug(_:)` helper) replaces 28 `print()` call sites across the Forge target; `peerProfileUpdated` guards on `peerIds.contains(peerId)`; merged two `keyboardWill{Show,Hide}` observers in `PlanSuggestionView`; deleted unreferenced `testingViews.playground`; removed a dead `isPlanNameFocused` commented line and a tombstone comment.
    - `GlobalSettings` gained `cornerRadiusSmall/Medium/Large` + `animationQuick/Standard/Slow` constants; 19 `.cornerRadius(...)` + 10 `.animation(...)` literal call sites migrated.
