...
Forge — minimal native iOS app (SwiftUI, Swift 5) for weightlifting. No external deps; only system frameworks (SwiftUI, Combine, UIKit, UserNotifications, ActivityKit, WidgetKit, HealthKit, WatchConnectivity). Dark-mode only. Apple Watch companion (haptic break-timer alerts) + self-hosted Swift server (`server/`) for collaborative workouts.

User flow: History → Plan Selector → Active Workout → back to History.
...
| View | Lines | Purpose |
|------|---|---------|
| `CompletedWorkoutsView` | ~125 | Home — history list, settings + Add Friend |
| `SettingsView` | ~95 | Theme picker + per-plan "Reset Last Completed" |
| `SelectPlanView` | ~250 | Manage plans — swipe-right share, swipe-left delete, drag reorder |
| `PlanEditorView` | ~310 | Create/edit plan — inline reorder/delete + swipe-right exercise transfer |
| `ExerciseEditorView` | ~410 | Add/edit exercises — coordinator |
| `HomogeneousSetPicker` | ~200 | 3-column wheel pickers (sets × weight × reps) |
| `HeterogeneousSetEditor` | ~235 | Per-set rows w/ weight/reps/till-failure |
| `WorkoutInProgressView` | ~860 | Active workout — Live Activity + HealthKit + stopwatch + break timer + collab + chat |
| `StartingCountdownView` | ~100 | 3s pre-workout countdown |
| `BreakTimerView` | ~155 | Configurable rest timer (`TimelineView`) + notification |
| `WorkoutBottomToolbarView` | ~120 | Add/Done/Edit (ignores keyboard) |
| `WorkoutChatToolbar` | ~70 | Mid-workout chat container — ZStack sibling of bottom toolbar; expands upward; rises with keyboard via parent's manual avoidance |
| `SetView` | ~140 | Reusable row. `Content` (`.individual`/`.summary`) × `Appearance` (`.standard`/`.muted`/`.workoutActive`/`.workoutActiveCollab`). |
| `HistoryView` | ~200 | Read-only past workout — calories/completion/duration |
| `ReorderDeleteView` | ~100 | Reorder/delete sheet |
| `BreakTimerWatchView` | ~110 | watchOS break timer — countdown ring + haptic |
| `WorkoutWithFriendView` | ~220 | Collab — Copy/Share. `activeWorkoutPlan` switches History entry vs mid-workout invite. |
| `ConnectingView` | ~75 | Collab — state-driven banner; auto-advances to `JoinSessionView` |
| `JoinSessionView` | ~220 | Collab — name + `PhotosPicker`. `onSoloProfileSaved` enables solo-profile-entry mode (mid-workout invite). |
| `PlanSuggestionView` | ~390 | Collab — carousel + chat + Ready. Auto-routes re-joiners via `workoutInProgress`. Shimmer via `TimelineView`. |
| `CollabChatPanel` | ~280 | Collab — reusable chat (scroll + input + `KeyboardPersistentTextView`). `showAvatarHeader` opt-in. Drag-down + scroll-down dismiss keyboard. Custom long-press iMessage-Tapback reaction picker. |
| `CollabStatusBanner` | ~170 | Collab — capsule overlay. `.waitingForPeer`/`.connecting`/`.disconnected`/`.error` + Re-invite. `peerExitInfo` override: "Friend finished/left". |
...
- `Application+build.swift` — HTTP `/` health + `WS /sessions/:id` upgrade + per-connection task group. Parses stable `?participantId=<uuid>` (UUID fallback for old clients). Handles `setReady`/`workoutFinished`/`workoutCancelled`/`goingBackground`/`returningToForeground`. Cleanup skips `removeParticipant` when `.away` so slot persists for reconnect.
- `Protocol.swift` — `ServerMessage`/`ClientMessage` enums + `Profile`/`PeerInfo`/`PlanSnapshot`. Codable + Sendable. Includes `peerAway`/`peerReturned`/`goingBackground`/`returningToForeground`. **Every protocol change requires server redeploy + app rebuild in lockstep.**
- `SessionManager.swift` — actor holding `[UUID: Session]`; lazy-creates on first `addParticipant`; deletes when empty. Per-session: `suggestedPlan`, `workoutInProgress` (set on all-ready transition; persists for session lifetime; cleared on finish/cancel), per-participant `isReady` + `state: ParticipantState (.connected/.away(since:))` + `awayTimeout: Task`. `addParticipant` returns `.added` (fresh slot → `peerJoined`) or `.rebound` (reconnect into `.away` slot → `peerReturned`). `setReady` returns true on just-became-all-ready. Hummingbird's autoPing default 30s.

**iOS (Forge target):**

- `Forge/View Model/SessionClient.swift` — `@MainActor` ObservableObject. State: `.idle`/`.connecting`/`.waitingForPeer`/`.paired(peerIds: [UUID])`/`.disconnected(reason:)`/`.error(_)`. Wire types: `Profile`, `PeerInfo`, `PlanSnapshot`, `ChatEntry` (`myReaction`/`peerReaction`; `id` is shared message id minted by sender, forwarded by server in `peerChat` so both peers key the same row), `PeerSetKey`, `UserPosition`, `PeerBreakTimer`, `PeerExitInfo`. **Stable participant ID:** `stableParticipantId` lazy var, persisted in UserDefaults `"collabParticipantId"`. Sent on every WS connect via `?participantId=<uuid>` so server rebinds reconnects to existing `.away` slots. Owned by `CompletedWorkoutsView` as `@StateObject` — NOT `ForgeApp` — and **injected at the NavigationStack root** so all destinations + modals inherit it. **Heartbeat:** `pingTask` calls `URLSessionWebSocketTask.sendPing` every 30s; pong-error → readLoop throws → `.disconnected` → auto-reconnect. **Auto-reconnect:** Combine sink schedules `reconnect()` with exponential backoff (2/4/8/16/32s capped); resets on `.welcome`. `reconnect()` AND `peerLeft` clear `peerPositions`/`peerBreakTimer`/`peerReady`/`peerAway`. Finish/Cancel/GoingBackground use awaited `task.send` so message flushes before close frame / iOS suspension.
- `Forge/Views/Collab/`:
  - `WorkoutWithFriendView.swift` — `activeWorkoutPlan: WorkoutPlan?` switches: nil (History) → `createSession()` + push `ConnectingView`; non-nil (mid-workout) → `startSharedSessionForActiveWorkout(plan:)` + dismiss. Single `.sheet(item: $activeSheet)` covers iOS share sheet AND workout-mode profile prompt (avoids two-`.sheet` conflict). When host has no profile in workout mode, presents `JoinSessionView(onSoloProfileSaved:)`; closure dismisses then re-fires action one runloop tick later (UIKit needs the hop).
  ...
  - `PlanSuggestionView.swift` — carousel + `CollabChatPanel()` + Ready + single `.fullScreenCover(item:)` enum-driven cover for `PlanEditorView` (preview) and `WorkoutInProgressView` (post-ready). `routeIntoActiveWorkoutIfNeeded()` (on appear + `onChange(of: workoutInProgress?.id)`) auto-routes re-joiners straight into active workout — they can't broadcast a fresh `startWorkout` and stomp the partner.
  - `CollabChatPanel.swift` — chat surface + `KeyboardPersistentTextView` (UIViewRepresentable wrapping `WrappingUITextView`, a UITextView subclass returning `noIntrinsicMetric` for width). `showAvatarHeader` opt-in renders peer-only avatar with away-dim treatment. `simultaneousGesture(DragGesture)` + `.scrollDismissesKeyboard(.immediately)` dismiss keyboard. **Custom reaction picker:** long-press peer bubble (0.3s) → medium haptic + spring-animates horizontal emoji capsule (six iMessage Tapbacks + X) as SIBLING of bubble in per-row VStack — normal layout flow below the bubble so next message gets pushed down. Tap emoji to apply (toggle off if same); X to dismiss. Reaction badge `Text(emoji).background(Circle())` with `.offset(x: ±12, y: 14)`; LazyVStack spacing 16 clears protrusion. Picker emoji buttons are plain `Text` + `.onTapGesture` (NOT `Button` — see gotcha).
  - `CollabStatusBanner.swift` — capsule overlay. Hidden in `.paired`/`.idle`. `.waitingForPeer` shows "Re-invite" that share-sheets EXISTING `sessionId` URL (does NOT call `createSession()`). **Clean-exit override:** `peerExitInfo` set → "Friend finished/left workout" with checkmark/xmark for ~5s then auto-dismisses; presence (even after auto-dismiss) suppresses disconnect/Re-invite chrome for rest of session. Does NOT surface peer backgrounding — that's avatar-level only.
  - `PlanCarouselCard.swift`, `PlanInfoBlock.swift` — shared plan-display pieces.
  - `CollabHelpers.swift` — `avatar`, `initial`, `resizeImage`, `ShareSheet`, `ShareableURL`, `awayDimmedAvatar` (opacity 0.45 + moon.fill badge for chat header; workout gutter uses inline opacity-only since small avatar makes badge cramped).
- `Forge/Views/History/CompletedWorkoutsView.swift` — Add Friend toolbar, `forge://session/<id>` routing in `.onOpenURL`. `.onChange(of: scenePhase)`: `.active` → `reconnect()` + `sendReturningToForeground()`, `.background` → `Task { await sessionClient.sendGoingBackgroundAndAwait() }` (must flush before iOS suspends). `.onChange(of: sessionClient.state)` → resets nav-destination flags on `.idle` (collapses entire collab stack to History). `NavigationStack` is closed by `.environmentObject(sessionClient)`.
- `Forge/Views/Workout/WorkoutInProgressView.swift` — gains `@EnvironmentObject var sessionClient`. In joint mode (`sessionClient.isPaired`) per-exercise cards have empty `Color.clear.frame(width: gutterWidth)` leading placeholder; `.overlayPreferenceValue(RowAnchorKey.self)` reads each row's bounds anchor and absolutely positions one `avatarColumn` per row at its measured `midY` — **wrap-resilient.** Peer avatars dim to 0.45 when `peerAway[peerId] != nil`. `myPosition` is `@State`; `recomputeMyPosition()` applies single rule (first-incomplete-set, with rest-row adjustment when active break timer's source set immediately precedes it). `.onChange(of: myPosition)` broadcasts; `.onChange(of: state)` matching `.paired` AND `.onChange(of: peerAway)` going non-empty → empty BOTH call `rebroadcastJointStateForPeer()`. `finishWorkout`/`cancelWorkout` send via async `sendAndAwait`; disconnect DEFERRED inside the 2s confetti `asyncAfter` so animation completes before nav unwinds. Mid-workout invite button (`person.2.fill`, visible when `sessionId == nil` OR `peerExitInfo != nil`). Mid-workout chat: `WorkoutChatToolbar` as ZStack sibling of `WorkoutBottomToolbarView`; manual keyboard avoidance shrinks `chatPanelHeight` so avatar at top edge stays fixed while bubbles area compresses. Anchor-preference types in sibling `WorkoutAvatarGutter.swift`.
...
- **iOS backgrounding kills WebSockets within seconds (iOS 18 reliably; iOS 26 sometimes survives a brief blip).** Mitigations: (a) server lazy-creates sessions on first WS connect; (b) client auto-reconnects via `.onChange(of: scenePhase)`; (c) the **away/return architecture** below makes brief backgrounds invisible to peer.
- **Background-aware presence (away/return).** On `scenePhase → .background`, client awaits `goingBackground` (must flush during iOS's ~5s grace). Server marks slot `.away`, broadcasts `peerAway` (NOT `peerLeft`), starts 5-min timeout. If client reconnects within window, `?participantId=<uuid>` (stable per-device UUID in UserDefaults `"collabParticipantId"`) lets `addParticipant` return `.rebound` — continuation rebound, timeout cancelled, broadcast `peerReturned`. If timeout fires, server promotes to real `peerLeft`. Other phone shows dimmed-avatar + moon-badge for `.away`; loud "Friend disconnected" banner only for true disconnects (force-quit, network drop, crash — none sent `goingBackground`).
- **`peerReturned` re-broadcast must trigger from the still-paired peer, NOT the returning peer.** When OTHER peer was `.away` and returns, my `state` stayed `.paired` throughout, so `.onChange(of: state) { case .paired }` doesn't fire on my side. But returning peer's `reconnect()` cleared their per-peer dicts — they need ME to replay. `WorkoutInProgressView` has a SECOND observer on `peerAway` firing `rebroadcastJointStateForPeer()` when dict transitions non-empty → empty. Without this, on iOS 18 peer's avatar disappears from gutter until they next complete a set.
...
- **`PlanSnapshot.toWorkoutPlan()` must preserve UUIDs on plan + each exercise**, or joint-mode set sync can't address rows across phones. `Exercise.init`/`WorkoutPlan.init` generate fresh UUIDs — extension copies `id` from snapshot after construction. Set positions use positional index (`SetSnapshot` has no id), so set-completion sync keys off `(exerciseId: UUID, setIndex: Int)`.
- **SwiftUI `TextField` with `.submitLabel(.send)` + `.onSubmit` always dismisses keyboard before handler fires** — visible flicker even with immediate `@FocusState` re-assertion. `KeyboardPersistentTextView` is `UIViewRepresentable` wrapping `UITextView` (multi-line); `textView(_:shouldChangeTextIn:replacementText:)` intercepts `"\n"` to fire `onSubmit` and returns `false`, keeping first-responder. Bonus: `autocorrectionType = .no` + `spellCheckingType = .no` + `smartInsert/Dashes/QuotesType = .no` reliably suppress QuickType + smart punctuation — SwiftUI's `.autocorrectionDisabled()` is inconsistent.
...
- **Don't use `.contextMenu` for the chat reaction picker.** We tried; the native lift/dismiss pipeline produced platform-specific glitches we couldn't reach from SwiftUI: badge clipping during lift snapshot (snapshot clips at receiver's layout bounds, so any `.offset` on badge gets cut off), iOS 18 bubble wobble during dismiss (runs at UIKit layer outside SwiftUI's transaction propagation; only fix is keeping receiver's frame constant via always-on padding), behind-the-bubble flash on stamp (badge sits in regular layer below the elevated platter), rounded-corner flicker. Each fix exposed another. Custom long-press picker (`onLongPressGesture` + sibling capsule view in per-row VStack) sidesteps all of it. **Don't "fix" this back to native.**
...
- **Small-screen UX swap (`isSmallScreen`).** `WorkoutInProgressView` defines `isSmallScreen: Bool { screenHeight < 700 }` (true on SE-class). On these, the break-timer panel can't fit the in-ring X cancel button, so the disabled back chevron at top-left swaps to an `xmark` calling `dismissBreakTimerView()` while timer is active, and in-ring X hides. Reverts when timer dismisses/expires. `isSmallScreen` is passed to `BreakTimerView` as a parameter so parent's screen-size determination flows consistently.
...
- **Don't put workflow-critical `@StateObject`s on the `App` struct if any view's `onAppear` resets navigation flags.** Promoting `PlanViewModel` to `@StateObject` on `ForgeApp` caused Scene body to re-evaluate on every publish, re-firing `CompletedWorkoutsView.onAppear` and resetting `isSelectPlanViewActive = false` — popping the workout fullScreenCover every time a plan was tapped. Working pattern: inline `.environmentObject(PlanViewModel())` in `ForgeApp.body` + `@EnvironmentObject` in views. For app-wide hooks (`.onOpenURL`), attach to root *view*, not Scene.
...
- [ ] **Mid-workout chat enhancements** — chat panel + reactions shipped; remaining:
  - [ ] Notifications (peer chat → local notification when backgrounded; coordinate with `goingBackground` away-mode flow)
- [~] **UI refactor: multi-iPhone-size support** — partial. Target: SE 3rd gen (375pt) → 17 Pro Max (430pt), portrait-only.
  - **Shipped:** Phase A constant annotations. B1 `WorkoutBottomToolbarView` `maxWidth: .infinity` columns. B2 `minimumScaleFactor` + `lineLimit(1)` on ≥30pt fonts. B3 `ExerciseEditorView` content-aware `.height(N)` detent + relative TextField. Plus: `BreakTimerView` ring sizing (200pt SE / scaled 260pt cap on others) + small-screen back-button-X swap, `WorkoutWithFriendView` divider gated to ≥414pt, "Change Specific Sets" toggle width 235→280, bottom-toolbar centering. Outer horizontal padding 40→24 on `StartingCountdownView` + `BreakTimerView`.
  - **Outstanding:** B4 (`WorkoutWithFriendView` Pro Max vertical-spacing — paused, manual). B5 (replace remaining `UIScreen.main` — see gotcha for sites). B6 (verify `SetView` strikethrough alignment on SE / Pro Max).
...
- [ ] **Production-grade server deployment** — Cloudflare Named Tunnel + launchd auto-start. Steps:
  ```bash
  cloudflared tunnel login                                       # one-time
  cloudflared tunnel create forge-ws
  ...
  ```
  Then change `SessionClient.serverHost` to `forge-ws.ryan-div.com`. Add launchd plists at `~/Library/LaunchAgents/com.ryandiv.forgeserver{,-tunnel}.plist` (`RunAtLoad + KeepAlive`).
